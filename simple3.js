...
function makeBtn(t, fn) { const b = document.createElement('button'); b.textContent = t; b.style.margin = '5px'; b.style.padding = '10px 12px'; b.style.border = '0'; b.style.borderRadius = '10px'; b.style.background = '#d8953f'; b.style.color = '#201209'; b.style.fontWeight = '900'; b.style.cursor = 'pointer'; b.onclick = fn; return b; }
function showMessage(title, txt) { card.style.display = 'block'; card.textContent = ''; const h = document.createElement('h2'); h.textContent = title; h.style.margin = '0 0 8px'; const p = document.createElement('p'); p.textContent = txt; p.style.lineHeight = '1.45'; card.append(h, p, makeBtn('Kapat', () => card.style.display = 'none')); }
function intro() { showMessage('Bölüm 1: Sancaklar Ayrıldı', 'Artık tek saray yok. Dünya haritası gerçek asset bölgelerine bölündü. Hareket sistemi de yenilendi: karakter artık düz kaymak yerine yürürken gövde salınımı ve adım hissi verir. WASD ile yürü, Shift ile koş, mouse ile kamerayı çevir, E ile konuş.'); }
function final() {
  ...
  card.style.display = 'block'; card.textContent = ''; const h = document.createElement('h1'); h.textContent = title; const p = document.createElement('p'); p.textContent = txt; card.append(h, p, makeBtn('Baştan Oyna', () => location.reload()));
}
const story = {
...
function openNpc(n) { const d = n.userData; state.visited[d.id] = true; card.style.display = 'block'; card.textContent = ''; const h = document.createElement('h2'); h.textContent = d.name + ' - ' + d.role; const p = document.createElement('p'); p.style.lineHeight = '1.45'; card.append(h, p); if (d.id === 'king' && score() >= 145) { p.textContent = 'Divan hazır. Sancaklar sözünü söyledi. Taht iddianı açıklayabilirsin.'; card.append(makeBtn('Final Divanı Topla', () => { apply({ trust: 8, inf: 6 }); final(); }), makeBtn('Biraz daha hazırlan', () => card.style.display = 'none')); return; } const s = story[d.id] || { t: 'Bu kişi saray dengesini etkiler.', o: [['Destek iste', { inf: 6, pub: 4 }]] }; p.textContent = s.t; s.o.forEach(o => card.appendChild(makeBtn(o[0], () => { apply(o[1]); openNpc(n); }))); card.appendChild(makeBtn('Ayrıl', () => card.style.display = 'none')); }
function setAction(root, moving) { const a = root.userData.actions; if (!a) return; const idle = a.idle, walk = a.walk; if (walk) { walk.setEffectiveWeight(moving ? 1 : 0); walk.timeScale = moving ? 1.25 : .2; } if (idle) idle.setEffectiveWeight(moving && walk ? .08 : 1); }
function animateCharacter(root, moving, dt) {
  ...
  if (moving) {
    walkPhase += dt * 11.5;
    root.position.y = baseY + Math.abs(Math.sin(walkPhase)) * 0.09;
    root.rotation.z = THREE.MathUtils.lerp(root.rotation.z, Math.sin(walkPhase) * 0.035, 0.32);
    root.rotation.x = THREE.MathUtils.lerp(root.rotation.x, Math.cos(walkPhase) * 0.018, 0.24);
  } else {
    idlePhase += dt * 2.0;
    root.position.y = THREE.MathUtils.lerp(root.position.y, baseY + Math.sin(idlePhase) * 0.018, 0.08);
    root.rotation.z = THREE.MathUtils.lerp(root.rotation.z, 0, 0.12);
    root.rotation.x = THREE.MathUtils.lerp(root.rotation.x, 0, 0.12);
  }
...
function loop() {
  ...
  if (ctx.mixers) for (const m of ctx.mixers) m.update(dt);
  const v = new THREE.Vector3((keys.KeyD ? 1 : 0) - (keys.KeyA ? 1 : 0), 0, (keys.KeyS ? 1 : 0) - (keys.KeyW ? 1 : 0));
  let moving = false;
  if (v.lengthSq()) {
    moving = true;
    v.normalize(); const f = new THREE.Vector3(Math.sin(angle), 0, Math.cos(angle)); const r = new THREE.Vector3(f.z, 0, -f.x);
  ...
  if (moving !== wasMoving) wasMoving = moving;
  animateCharacter(ctx.player, moving, dt);
  for (const n of ctx.npcs) animateCharacter(n, false, dt);
  let best = null, dist = 999;
  for (const n of ctx.npcs) { const d = n.position.distanceTo(ctx.player.position); if (d < dist) { dist = d; best = n; } }
...
ctx.camera.position.set(0, 16, 104); ctx.camera.lookAt(ctx.player.position); hudUpdate(); intro(); loop();
